...
class WorkflowStepExecution:
    ...
    def _db_workflow_step(self):
        try:
            return self.db_session.query(MdWorkflowStep).filter(MdWorkflowStep.workflow_step_pk == self.db_object.workflow_step_fk).first()
        except Exception as e:
    ...
    def get_learned_instructions(self):
        ...
        try:
            previous_steps_execution = self.db_session.query(MdUserWorkflowStepExecution)\
                ...
                .filter(MdUserWorkflowStepExecution.user_workflow_execution_fk == self.db_object.user_workflow_execution_fk)\
                .filter(MdUserWorkflowStepExecution.user_workflow_step_execution_pk != self.db_object.user_workflow_step_execution_pk)\
                .all()
            return [{'result': previous_step_execution.result, 'instruction': previous_step_execution.learned_instruction} for previous_step_execution in previous_steps_execution if previous_step_execution.learned_instruction and previous_step_execution.parameter==self.parameter]
        except Exception as e:
