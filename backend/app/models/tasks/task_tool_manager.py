...
class TaskToolManager:
    ...
    def manage_tool_usage_text(self, text, user_task_execution_pk, task_tool_associations_json):
        try:
            mojo_text = AssistantMessageGenerator.remove_tags_function(text, self.tool_usage_start_tag, self.tool_usage_end_tag)
            tool_name = AssistantMessageGenerator.remove_tags_function(text, self.tool_name_start_tag, self.tool_name_end_tag)
