...
from mojodex_core.entities import MdUserTask, MdUserWorkflowStepExecutionResult, MdWorkflowStep, MdUserWorkflowStepExecution
...
class WorkflowExecution(UserTaskExecution):
    ...
    def __init__(self, workflow_execution_pk):
        try:
            super().__init__(workflow_execution_pk, Session(engine))
            self.user_id = self.user_task.user_id
            self.past_accepted_steps_executions = [WorkflowStepExecution(self.db_session, db_past_accepted_step_execution, self.user_id, self.task.name_for_system) for
    ...
    def _db_past_accepted_step_executions(self):
        try:
            return self.db_session.query(MdUserWorkflowStepExecution) \
                ...
                .filter(MdUserWorkflowStepExecution.user_task_execution_fk == self.user_task_execution_pk) \
    ...
    def _generate_new_step_execution(self, step, parameter: dict):
        try:
            ...
            return WorkflowStepExecution(self.db_session, db_workflow_step_execution, self.user_id, self.task.name_for_system)
        except Exception as e:
    ...
    def _get_next_step_execution_to_run(self):
        try:
            if not self.past_accepted_steps_executions:  # no step validated yet
                self._current_step = self._generate_new_step_execution(self.task.db_steps[0],
            ...
            if len(self.past_accepted_steps_executions) > 1:  # no dependency as it was the first step
                db_dependency_step = self.db_session.query(MdWorkflowStep) \
                    ...
                    .filter(MdUserTask.user_task_pk == self.user_task_fk) \
                ...
                db_dependency_step_execution = self.db_session.query(MdUserWorkflowStepExecution) \
                    .filter(
                    MdUserWorkflowStepExecution.user_task_execution_fk == self.user_task_execution_pk) \
    ...
    def initial_parameters(self):
        # self.user_task_inputs is [{"input_name": "<input_name>", "default_value": "<value>"}]'
        # initial_parameters is {"<input_name>": "<value>", ...}
        try:
            return {input["input_name"]: input["value"] for input in self.user_task_inputs}
        except Exception as e:
    ...
    def run(self):
        try:
            if not self.title:
                server_socket.start_background_task(self.generate_title_and_summary)
            ...
            socketio_message_sender.send_error(f"Error during workflow run: {e}", self.session_id,
    ...
    def _generate_produced_text(self):
        try:
            ...
            last_step= self.task.db_steps[-1]
            db_validated_last_step_executions = self.db_session.query(MdUserWorkflowStepExecution) \
               ...
               .filter(
                ...
                .order_by(MdUserWorkflowStepExecution.creation_date.desc())\
                .all()
            validated_last_step_executions = [WorkflowStepExecution(self.db_session, step, self.user_id, self.task.name_for_system) for step in db_validated_last_step_executions]

            production = "\n\n".join([list(step.result[0].values())[0] for step in validated_last_step_executions[::-1]])

            produced_text_manager = WorkflowProducedTextManager(self.session_id, self.user_id,
                                                                self.user_task_execution_pk)
            produced_text, produced_text_version = produced_text_manager.save(production, text_type_pk=self.task.db_object.output_text_type_fk,)
    ...
    def _send_ended_step_event(self):
        try:
            ...
            step_execution_json["session_id"] = self.session_id
            server_socket.emit('workflow_step_execution_ended', step_execution_json, to=self.session_id)
        except Exception as e:
    ...
    def get_step_execution_from_pk(self, step_execution_pk: int) -> WorkflowStepExecution:
        try:
            db_step_execution = self.db_session.query(MdUserWorkflowStepExecution) \
                ...
                .first()
            return WorkflowStepExecution(self.db_session, db_step_execution, self.user_id, self.task.name_for_system)
        except Exception as e:
    ...
    def invalidate_current_step(self, learned_instruction):
        try:
            ...
            current_step_in_validation.invalidate(self.session_id)
            ...
            for validated_step in self.past_accepted_steps_executions[checkpoint_step_index:]:
                validated_step.invalidate(self.session_id)
