...
class KnowledgeCollector:
    ...
    def __init__(self, user_name, user_timezone_offset, user_summary, user_company_knowledge, user_business_goal):
        ...
        self.user_datetime_context = self.get_global_context_knowledge(user_timezone_offset if user_timezone_offset else 0)
    ...
    def get_global_context_knowledge(timezone_offset=0):
        ...
        user_datetime_context = MPT("mojodex_core/instructions/user_datetime_context.mpt",
                             weekday=timestamp.strftime("%A"),
                                   datetime=timestamp.strftime("%d %B %Y"),
                                   time=timestamp.strftime("%H:%M"))
        return user_datetime_context.prompt
    ...
    def localized_context(self):
        try:
            ...
            return MPT("mojodex_core/instructions/user_datetime_context.mpt",
                                weekday=timestamp.strftime("%A"),
                                    datetime=timestamp.strftime("%d %B %Y"),
                                    time=timestamp.strftime("%H:%M"))
