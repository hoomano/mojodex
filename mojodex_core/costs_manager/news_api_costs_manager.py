...
class NewsAPICostsManager(CostsManager):
    ...
    def __init__(self):
        try:
            ...
            EmailService().send_technical_error_email(f"{NewsAPICostsManager.logger_prefix}: __init__: {e}")
    ...
    def on_search(self, user_id, num_of_results_asked, user_task_execution_pk, task_name_for_system):
        try:
            ...
            EmailService().send_technical_error_email(f"{NewsAPICostsManager.logger_prefix}: on_search: {e}")