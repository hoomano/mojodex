...
class SendDailyEmails(ScheduledTask):
    logger_prefix = "SendDailyEmails"

    def job(self, offset=0, batch_size=50):
        try:
            uri = f"{os.environ['MOJODEX_BACKEND_URI']}/send_daily_emails"
            ...
            if internal_request.status_code != 200:
                self.logger.error(f"Error sending daily emails : {internal_request.text}")
            else:
                user_ids = internal_request.json()['user_ids']
                self.logger.info(f"Daily emails successfully launched. {len(user_ids)} users are concerned.")
